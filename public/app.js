...
class AppState {
  constructor() {
    ...
    this.exchangeRate = 0.012; // INR to USDT rate
  ...
  updateAuthUI() {
    ...
    if (this.currentUser) {
      ...
      document.getElementById('userAvatar').src = 
        this.currentUser.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(this.currentUser.email)}&background=014666&color=fff`;
    } else {
  ...
  updateProfilePage() {
    ...
    profileAvatarImg.src = this.currentUser.photoURL || 
      `https://ui-avatars.com/api/?name=${encodeURIComponent(this.currentUser.email)}&background=014666&color=fff&size=80`;
  ...
  async fetchExchangeRate() {
    try {
      ...
      this.exchangeRate = 0.012; // Fallback rate
      this.updateRateDisplay();
    }
  }

  updateRateDisplay() {
    document.getElementById('exchangeRate').textContent = this.exchangeRate.toFixed(6);
    document.getElementById('currentRate').textContent = `Live FX rate: ₹1.00 = ${this.exchangeRate.toFixed(6)} USDT`;
  }
  ...
  updateConverter() {
    ...
    document.getElementById('receiveAmount').textContent = receiveAmount.toFixed(6);
  }
  ...
  async initiateTransfer() {
    ...
    // Show payment modal
    this.showPaymentModal(sendAmount, receiveAmount);
  }

  showPaymentModal(inrAmount, usdtAmount) {
    document.getElementById('paymentINR').textContent = `₹${inrAmount.toLocaleString()}`;
    document.getElementById('paymentUSDT').textContent = `${usdtAmount.toFixed(6)} USDT`;
    document.getElementById('paymentRate').textContent = 
      `₹1.00 = ${this.exchangeRate.toFixed(6)} USDT`;
    ...
    document.getElementById('proceedPayment').onclick = () => {
      this.processPayment(inrAmount, usdtAmount);
    };
  }

  async processPayment(inrAmount, usdtAmount) {
    this.showLoading(true);
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          await this.verifyPayment(response, inrAmount, usdtAmount);
        },
        prefill: {
  ...
  async verifyPayment(paymentResponse, inrAmount, usdtAmount) {
    this.showLoading(true);
